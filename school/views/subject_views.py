...
class SubjectList(generics.ListAPIView):
    ...
    view_permissions = {"get": {"admin": True, "teacher": True, "student": True}}

    def get_serializer_class(self):
        teacher_id = self.request.query_params.get("teacher_id")
        if teacher_id:
            return (
                ClassSubjectSerializer  # Use ClassSubjectSerializer for teacher queries
            )
...
class OutlineList(generics.ListCreateAPIView):
    ...
    view_permissions = {"get,post": {"admin": True, "teacher": True, "student": True}}
        ...
        if class_id and subject_id:
            ...
            if _class and subject:
                class_subject = ClassSubject.objects.filter(
                    _class=_class, subject=subject
                ).first()
                if class_subject:
                    queryset = (
                        Outline.objects.filter(
                            class_subject___class=_class, class_subject__subject=subject
                        )
                        .order_by("week")
                        .distinct()
                    )
